...
def display_grid(grid):
    ...
    if st.session_state.is_session_pc:
        ...
        for j, row in enumerate(grid):
            sac.buttons(
                        [
                            list(row)[i] for i in range(len(row))
                        ],
                        align='center'
                        )
...
def display_options():
    ...
    if st.session_state.is_session_pc:
        ...
        sac.buttons(
            [
                list(st.session_state.shapes)[i] for i in range(len(st.session_state.shapes))
            ],key="shapesans",
            align='center'
        )
        check_answer(st.session_state["shapesans"],st.session_state.ans)
